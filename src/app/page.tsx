...
function HomeContent() {
  ...
  const contentMap = {
    'bg-about': (
      <div>
        <h1>VƯƠN TẦM</h1>
        <h2>BỀN VỮNG</h2>
        <p><span style={{color: '#ff671f'}}>MSB</span> kiên định mô hình ngân hàng hài hòa giữa hiệu quả kinh doanh và trách nhiệm xã hội.</p>
        <p><span style={{color: '#ff671f'}}>Phát triển bền vững</span> không chỉ là chiến lược, mà là <span style={{color: '#ff671f'}}>cam kết dài hạn của chúng tôi</span>.</p>
        <p>Tạo ra giá trị cho cả hiện tại và tương lai.</p>
      </div>
    ),
    'bg-dautu': (
      <div>
        <h1>CÙNG <br /> VƯƠN TẦM</h1>
  ...
  const getCurrentContent = () => {
    const bgKey = Object.keys(contentMap).find(key => backgroundImage.src.includes(key));
    return bgKey ? contentMap[bgKey as keyof typeof contentMap] : (
      <div>
